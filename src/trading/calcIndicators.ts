...
// Định nghĩa kiểu cho dữ liệu Volume Profile
interface VolumeProfileData {
  priceLevels: number[];
  volumes: number[];
}

// Định nghĩa kiểu cho dữ liệu tối ưu của chuỗi thời gian
interface OptimizedTimeSeries {
  last_value: number;
  slope: number;
}

// Định nghĩa kiểu cho dữ liệu tối ưu của Volume Profile
...
// Định nghĩa kiểu cho kết quả trả về của optimized_calculateIndicators
...
async function optimized_calculateIndicators(timeframe: string, candles: Candle[]): Promise<OptimizedIndicator[]> {
  const indicators: { [key: string]: any }[] = await calculateIndicators(timeframe, candles); // Gọi hàm gốc
  ...
  const optimizedIndicators: OptimizedIndicator[] = indicators.map((indicator: { [key: string]: any }) => {
    ...
    if (key === "Volume Profile") {
      ...
      return { [key]: data }; // Giữ nguyên, giả định đây là { [key: string]: number }
    } else {
